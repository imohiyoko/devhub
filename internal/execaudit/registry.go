...
var Registry = []Surface{
	...
	},
	{
		...
		Kind:     Fixed,
		Trigger:  "process startup — reclaims devhub's own TCP port from an orphaned prior instance",
		Input:    "port is devhub's own configured port (int); a candidate PID is killed only when its executable basename is exactly \"devhub\".",
		Gate:     "Not reachable over HTTP; runs in-process at boot only (unix; no-op where lsof/ps are absent).",
	},
	{
		ID:       "ports-kill",
	...
	},
	{
		...
		Kind:     Fixed,
		Trigger:  "GET /api/ports and the env-launcher's live-port index",
