...
var Registry = []Surface{
	...
	},
	{
		...
		Notes:    "Unix hands off with syscall.Exec (no exec.Command — not scanned by the guard); Windows uses exec.Command + Run and propagates the child's exit code. The provenance token is stripped before hand-off, so the target's argv is `start <flags>` and a subsequent self-restart stays on that provenance.",
	},
	{
		ID:       "workspace-editor",
