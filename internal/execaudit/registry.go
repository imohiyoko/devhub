...
var Registry = []Surface{
	...
	},
	{
		...
		Input:    "Fixed argv per operation. docker: `[--context colima-<profile>] compose --project-name <project> [--file <f>…]` followed by `ps --format json --all`, `up --detach --wait <services>` or `stop <services>`; project/files/cwd/services/profile come from the saved env definition, not from the request, and the profile name is restricted to [A-Za-z0-9_-] at save time. The capability probe is a bare `compose version --short`. colima: `list --json`, which takes no input at all. No command string is accepted from the caller on any path.",
		Gate:     "host allowlist + API token (/api); loopback + Sec-Fetch + manual approval (/ai-api). Writing env definitions goes through POST /api/envs (same gates).",
		Notes:    "Every docker invocation that touches containers is scoped to the definition's compose project name and to the services that definition declares, which is how devhub bounds itself to the containers the environment owns: it can start and stop those, and nothing else. The engine is selected with a per-command --context derived from the environment's Colima profile; `docker context use` is never run, so the user's other terminals keep whatever context they had (plan §6.3). An environment that declares no Colima profile passes no --context and uses the ambient one. The colima surface is read-only — devhub lists profiles and never starts, stops or reconfigures a VM. All calls funnel through execRunner in internal/controllers/envs/command.go, which is why one Surface covers every runtime adapter.",
	},
	{
		ID:       "git",
		Binaries: []string{"git"},
