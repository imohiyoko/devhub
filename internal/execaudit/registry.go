...
var Registry = []Surface{
	...
	},
	{
		...
		Input:    "Fixed argv: `start --profile <name> [--cpus N] [--memory N] [--disk N] [--runtime docker|containerd]`, and for a resize `stop --profile <name>` first. The name must match [A-Za-z0-9_-]+ and the engine must be one devhub has an adapter for, both checked before anything is spawned; the numbers are rendered from parsed integers, never passed through as text. --force is never passed to stop, so the guest shuts down gracefully.",
