...
switch ($Action) {
  ...
  'install' {
    ...
    # cmd.exe は .cmd を OEM コードページで読むため、シム本体と companion
    # PowerShell は ASCII のみにする。RepoRoot は UTF-8 base64 markerだけを
    # 埋め込み、実行時に PowerShell 側で復元するので日本語を含むパスも壊れない。
    $rootB64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($RepoRoot))
    $runner = Join-Path $binDir 'devhub-source.ps1'
    $runnerContent = @"
`$ErrorActionPreference = 'Stop'
`$repoRoot = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('$rootB64'))
Set-Location -LiteralPath `$repoRoot
& go run ./cmd/devhub @args
exit `$LASTEXITCODE
"@
    $content = @"
@echo off
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0devhub-source.ps1" %*
exit /b %ERRORLEVEL%
"@
